	...
	$(document).ready(function() {
		...
		$('form#'+formId).on('keyup change paste', 'input, select, textarea', function(){
			...
			if(str.length >0){
				...
				str.map(records=>{
					if(records.name){
						if(!tmpObj[records.name]){
							tmpObj[records.name] = {name: records.name, value: records.value};
						}else{
							let tmpValues = tmpObj[records.name].value;
							if(tmpValues.constructor != Array)  tmpValues = [tmpValues];

							tmpValues= tmpValues.concat([records.value]);
							tmpObj[records.name]= {name: records.name, value: tmpValues};
						}
					}
				});
				str = Object.values(tmpObj);
			}
